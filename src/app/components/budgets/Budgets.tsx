...
export function Budgets() {
  ...
  const totalAllocated = enrichedBudgets.reduce(
  ...
  if (loading) {
    return <div className="p-10 text-center">Cargando presupuestos...</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Disponible",
            value: formatCOP(Math.max(0, totalAllocated - totalSpent)),
          },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {enrichedBudgets.map((budget: any) => {
          const pct = budget.allocated
            ? Math.min(110, (budget.realSpent / budget.allocated) * 100)
            : 0;
          const { variant, label } = getBudgetStatus(pct);
          const remaining = budget.allocated - budget.realSpent;
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-card p-6 rounded-2xl max-w-sm w-full">
            <p className="mb-6">¿Eliminar este presupuesto?</p>
